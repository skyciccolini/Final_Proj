...
# A americanum plot for reconstruction age with both bacterias

# Add a new column to each dataset
PF_Rick_Restime_AA_preds$Pathogen <- "Francisella"
PF_Rick_Restime_AA_preds$Pathogen <- "Rickettsia"

# Combine the datasets
PF_paths_AA_combined_preds <- rbind(PF_Rick_Restime_AA_preds, PF_Rick_Restime_AA_preds)

# Updated ggplot
recon <- ggplot(PF_paths_AA_combined_preds, aes(x = Restime, y = predicted_prob, linetype = Pathogen)) +
  geom_line() +
...
# Combine the datasets
TP_paths_AA_combined_preds <- rbind(TP_Fran_FireTime_AA_preds, TP_Rick_FireTime_AA_preds)

# Updated ggplot
fire <- ggplot(TP_paths_AA_combined_preds, aes(x = FireTime, y = predicted_prob, linetype = Pathogen)) +
  geom_line() +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2) +
